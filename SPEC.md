...
- **Motion:** restrained and functional — motion exists to explain a state change, never to decorate. Allowed:
  ...
  - the notifications slide-over's enter/exit, on the same terms (added 2026-08-11, plan 064) — the toast's own fade is already covered by the transition bullet above, and its auto-dismiss is a timer, not an animation: it removes the element, it does not animate it out;
...
- **Doctor** (added 2026-08-11): a slide-over like the log panel, opened from a quiet **Doctor** button in the header. One section per registered project, in `projects.json` array order — never sorted by severity, for the same reason the grid is never re-sorted and the Ports panel is never sorted by state. A section lists what Hangar can already tell, *before* a Run, about whether that project would even start: key names declared in a `.env.example` / `.env.sample` / `.env.template` that are absent from `.env`; a `.nvmrc` pin that the Node on the §8 resolved PATH does not satisfy; whether the next Run will install first (the same §9 step 3 decision, shown earlier rather than recomputed); and a project folder that is gone (§12 already warns on the card — the panel restates it so one place lists everything). A finding is one line and carries exactly four things: a stable id, a severity of **blocker · warning · note**, a human sentence, and the file it came from. A project with nothing to report says so once, quietly — no score, no badge, no percentage, because a check that celebrates itself becomes noise the user learns to skip. A project with no example env file is never asked about env keys, and one with no `.nvmrc` is never asked about Node: Hangar does not invent a policy the project never had. A missing folder, an unreadable `.env` and a malformed `package.json` are **findings, not errors** — a toast per project on open would be intolerable. It is a **snapshot, not a monitor**, on exactly the Ports panel's terms: it reads once on open and again only on Refresh, it never polls, nothing it does runs before the grid renders, and the header states when the snapshot was taken. Esc closes it on the same terms as the other slide-overs. **It reads `.env` files for KEY NAMES ONLY — a value is never retained, never serialized, never logged, never rendered, and no type in the report has a field capable of holding one.** **It carries no control that changes anything**: no fix, no install, no "create it for me", no link that writes — it reads and reports, and every remedy stays the user's own action in their own editor. And **preflight never blocks Run**: a finding never gates, delays, confirms or reorders §9, which behaves exactly as it does today whether this panel has ever been opened or not.
- **Notifications** (added 2026-08-11, plan 064): **toasts auto-dismiss, and a bell keeps them.** A toast used to sit on screen until it was clicked, so "Moved to folder" — a confirmation of something the user had just done and already knew — cost a click to get rid of, which is the interaction cost of an error applied to a success. A toast now leaves on its own: **4 s for the neutral tone, 6 s for the error tone**, an error carrying more words and a possible action and so needing longer to read. **Hovering the toast pauses the countdown and leaving resumes it from where it stopped** — not from the start — and keyboard focus inside the toast pauses it on the same terms, because a message that vanishes mid-sentence while it is being read is the single most irritating version of this feature. The manual dismiss button stays, a new toast still replaces the current one, and an identical message raised twice gets its own full countdown rather than inheriting the tail of the first one's. **These two halves are one feature, and neither ships alone.** §7 makes toasts the only surface for every command error, and §9/§12 put real, actionable text in them — the port-collision message carries a **Ports** button, the install-failure message carries a **Show logs** button — so auto-dismiss *without* somewhere to retrieve them would destroy information the app used to keep. The bell is therefore not a nice extra; it is the thing that makes the timer safe. It is a quiet header control beside Ports, Doctor and Inbox, carrying **an unread count when there are unread entries and nothing at all when there are none**, and it opens a slide-over on exactly the terms §11 already sets for those three: it reads local state only, makes no command call, no fetch and no network request of any kind — there is nothing here that *could* poll — Esc closes it, and it joins both the `inert` overlay set and the folder band's Esc guard, because one keypress may never fire two unrelated state changes. Unlike Ports, Doctor and Inbox it is **not hidden when the registry is empty**: those three would have nothing to show without projects, while this one is the retrieval surface for §7's only error channel, and a control that first appears *after* a message has already been dismissed is a control the user has never seen at the moment they need it. Each entry keeps its **text, tone, relative time and its action button** — Ports and Show logs work from the panel exactly as they do from the toast, derived by the same predicate in both places so the two can never drift, and re-derived live so a removed project never renders a stale name and a project that has since started never still claims a port collision. The list is **newest first**. **That is deliberate, and it is the one list in this app that is not in `projects.json` array order** — every other ordered surface (the grid, the Ports panel, the Doctor panel, the launch line) is a *registry* and is never re-sorted, whereas this one is a *chronology* answering "what just happened", where the most recent entry is the one being looked for. It is **capped** at a fixed recent count (100), oldest falling off first, never the newest. **Repeated messages are never deduplicated**: two identical "port is in use" rows are two real events, and collapsing them into one row with a count would hide a retry loop, which is precisely the thing that needs to be seen. The panel carries **no control that changes anything** beyond the two actions the toasts themselves already carried — no mark-as-read beyond opening it, no filter, no search, no severity beyond the existing two tones, and no clear. It is **ephemeral**: held in memory for the session, **never written to disk** — no `projects.json` field, no settings key, no new file — so §5's data model does not change for a notification log, and quitting Hangar clears it. There are **no desktop or OS notifications, no sound and no dock badge** (§3). Finally, the rule the whole element turns on: **§7 says errors surface as toasts, and that is still true — the bell is where a toast goes, not a replacement for it.** If anything ever raises an error into the history *without* also raising the toast, an error can be missed entirely, which is strictly worse than the behaviour this replaced.
