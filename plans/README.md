...
| 054 | GitHub notification list — slice 2 | — | P2 | L | 053 | **SUPERSEDED by 062** — its premise was measured and found false: 50 of 50 unread notifications are CI failures, 0 issues, 0 PRs |
| 055 | GitHub read + reply — slice 3 | — | P2 | M | 054 | **DEFERRED** — nothing to read yet (0 issues, 0 PRs, 0 stars). Trigger to revisit: the first real issue or comment from a stranger |
...
| 062 | Inbox rebuilt around build state — supersedes 054 | — | **P1** | M | 053 | DONE (2026-08-11) — 218 tests, was 191; **"could not run" is never green, mutation-tested in two directions** (inconclusive conclusion, and a truncated page) — reviewer re-ran the truncation one; §18 invariants unmoved (no `From<GithubError> for String`, `expose()` still 2 production sites, ACL byte-unchanged, no write path); repo detection reads `.git/config`, **no subprocess**; **verified against fixtures only, never the live API** |
| 063 | Tell the user when they are running an old build | — | **P1** | S | — | DONE (2026-08-11) — 199 tests; **tolerance mutation-tested** (strict `>` → RED), reviewer re-ran it; **text only** — no restart, kill, download or version server, and a test asserts the payload cannot carry one; **reference point is the later of the `build.rs` stamp and the exe mtime at startup**, because the constant alone permanently false-nags wherever the bundle was copied after being built; **re-read on window focus**, a stated departure from plan 060 |
| 064 | Toasts auto-dismiss + a bell that remembers them | — | **P1** | M | — | DONE (2026-08-11) — **zero `.rs` files touched**, Rust tests unchanged at 226/3; **auto-dismiss and the history shipped as one change**, because §7 makes toasts the only error surface and the Ports (047) / Show logs (034) buttons would otherwise become unreachable; 4 s neutral / 6 s error, **hover and keyboard focus pause the countdown and it resumes from the remaining time**, not from the start; new zero-import leaf `src/notifications.ts` + 17 `node --test` cases; **"opening the panel zeroes the unread count" mutation-tested** (RED on 3 cases, GREEN restored); **newest-first — the one list not in `projects.json` array order**, stated in §11 so it does not read as an oversight; **never deduplicated** — two identical "port is in use" rows are two events; **nothing written to disk**, no new settings key, no `projects.json` field; the two action buttons are derived by **one shared predicate** used by both the toast and the panel, re-derived live so a removed project shows no stale name; **not visually verified — the executor is headless and cannot watch a timer expire** |
